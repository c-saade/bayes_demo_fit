...
It is always a good idea to check the validity of our priors by making prior predictions, _i.e._, taking a look at model predictions when sampling parameters from the prior and checking that data falls into the range predicted by the prior distributions:
...
The observed data falls well within the range of the prior predictions, so we can be confident in our prior choices.
...
To do so, we will sample 1000 parameter values from the posterior distributions and simulate the resulting dynamics.
...
In particular, _Blepharisma_ sp. experiences very low competition but exerts a strong competition on _Tetrahymena_ sp., which explains why the former ends up excluding the latter.
...
If you want to delve into the specific of such a model, I invite you to check out the [code](https://doi.org/10.5281/zenodo.6364903) for the paper from which the data comes.
